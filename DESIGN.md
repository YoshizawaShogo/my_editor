...
- **オートインデント**: `Enter`でカーソル位置までの先頭空白を引き継ぐ。インデント途中で分割しても後半の既存空白を重複させない。カーソル直前（行末空白は無視）が`{`・`(`・`[`なら、引き継いだインデントに1段（`tab_size`個のspace、`insert_spaces=false`なら実タブ1文字）足す。現在行が言語設定の`line_comment`（`//`/`#`等）なら、同じインデント＋コメント記号＋空白を次行へ継続する。`///`/`//!`も維持する。
...
- Ctrl+クリック: 定義ジャンプ（LSP。専用キーは無い）。左右分割中はクリックした側と反対のペインに開き、使用箇所と定義を並べて見せる。さらに辿ると開く側が交互になり、常に直前の階層が隣に残る。
...
| ドラッグ | 範囲選択 |
| ホイール | スクロール（キーボードのページ送りは持たない） |
| Ctrl+クリック | 定義ジャンプ（LSP。専用キーは無い）。左右分割中は反対側のペインに開く |
| Alt+クリック | カーソル追加（マルチカーソル） |
