...
- 上記対応後は単体テスト149件、`cargo clippy --all-targets -- -D warnings`、`git diff --check`が成功した。
...
- bracketed pasteを有効化し、外部複数行ペーストを改行インデント処理から分離した。シェルは10,000行のスクロールバックをホイール操作できる。
- 括弧・引用符のペア補完、空ペアBackspace、soft-tab単位Backspace、閉じ波括弧のインデント調整、対応括弧と表示範囲内の同一語ハイライトを追加した。
- `use`/`import`文の関数補完では括弧を追加せず、自動補完失敗をステータスへ出さないようにした。診断はLazyVimの規則を基準に全severityのガター、範囲下線、`●`付き仮想テキストへ統一した。
- 検索ペインは`Ctrl+C`では閉じず、`Esc`だけで閉じるようにした。
- 検索・置換ペインから`Ctrl+T`等のPickerを開く際は検索状態を閉じ、文字・上下・確定操作がPickerへ届くよう補助UIを排他化した。
- `Ctrl+T`/`Ctrl+G`は同じPickerの再押下で閉じ、Picker外の左クリックでも閉じるようにした。再オープン時は走査tokenを照合し、閉じる前の古いファイル走査結果を破棄する。
- サイドボタン8/9が実機で機能せず未使用だったため、専用のundo/redo対応とcrosstermのローカルforkを削除し、crates.io版へ戻した。
- `()`・`[]`・`{}`の空ペア内でEnterを押した時は2回改行し、内側の入力行を1段深く、閉じ括弧を元のインデントへ配置するようにした。
- warning診断の行ガター記号を、小さい三角形`▵`へ変更した。
- 上記対応後は単体テスト153件、`cargo clippy --all-targets -- -D warnings`、`git diff --check`が成功した。
